...
from ...domains import Capacitor
from .component_configs import COMPONENT_COLUMNS_INDEX

class CapacitorSerializer(ComponentSerializer):
    def __init__(self) -> None:
        super().__init__()

    def load(self, query_result: list[str]) -> Capacitor:
        capacitance = float(query_result[COMPONENT_COLUMNS_INDEX["special_attr"]])
    ...
    def load_many(self, query_results: list[list[str]]) -> list[Capacitor]:
        return super().load_many(query_results)
    
    def dump(self, obj: Capacitor) -> list[str]:
        data = super().dump(obj)
        data.append(obj.get_capacitance())
        return data