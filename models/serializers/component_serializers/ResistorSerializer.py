...
from ...domains import Resistor
from .component_configs import COMPONENT_COLUMNS_INDEX

class ResistorSerializer(ComponentSerializer):
    def __init__(self) -> None:
        super().__init__()

    def load(self, query_result: list[str]) -> Resistor:
        resistance = float(query_result[COMPONENT_COLUMNS_INDEX["special_attr"]])
    ...
    def load_many(self, query_results: list[list[str]]) -> list[Resistor]:
        return super().load_many(query_results)
    
    def dump(self, obj: Resistor) -> list[str]:
        data = super().dump(obj)
        data.append(obj.get_resistance())
        return data